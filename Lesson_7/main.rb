...
class Main
  ...
  def initialize
    @stations = [Station.new("ййй"),Station.new("ккк"),Station.new("еее"),
    Station.new("ннн"),Station.new("гггг")]
    @trains = [PassengerTrain.new('333 33'),PassengerTrain.new('222 22'),
    PassengerTrain.new('444 44')]
    @routes = [Route.new("ттт", "т"),Route.new("ппп", "п")]
  end
  ...
  def create_train
    ...
    begin 
      ...
      if choose_type.to_i == 1
        @trains << PassengerTrain.new(id_train) 
      else
        @trains << CargoTrain.new(id_train) 
      end
  ...
  def delete_wagon
    ...
    current_train(num_train).show_wagons.each_with_index { |wagon, index| print " #{index + 1}: #{wagon}, тип: #{wagon.type}\n" }
    ...
    loop do
      ...
      if current_train(num_train).wagons.empty?
        ...
        when 1  
          ...
          current_wagon.occupied_volume(volume)
          puts "В вагоне #{current_wagon} осталось: #{current_wagon.free_volume} места!"
          break
        when 2
          puts "Выберите номер вагона:"
          num_wagon = gets.chomp.to_i
          current_wagon = current_train(num_train).wagons[num_wagon - 1]  
          puts "Вагоне #{current_wagon}, занятый объем состовляет: #{current_wagon.current_volume} места!"
          break
        when 3
          puts "Выберите номер вагона:"
          num_wagon = gets.chomp.to_i
        ...
        when 2
          ...
          puts "В вагоне #{current_wagon} занято мест: #{current_wagon.occupied_seats}"
          break
        when 3
          puts "Выберите номер вагона:"
          num_wagon = gets.chomp.to_i
          ...
          puts "В вагоне #{current_wagon} осталось мест: #{current_wagon.free_seats}" 
          break
        else
          puts "Такой пункт меню отсутствует!"
        end
